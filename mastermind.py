...
def create_code():
    ...
    #global code
    code = [0, 0, 0, 0]
    #  try this if all else fails.... just cause it is something XD
    #                           code = list()
    #                           while len(code) < 4
...
def run_game():
    ...
    while not correct and turns < 12:
        # was:         correct_digits_only, correct_digits_and_position = take_turn(code,answer)
        correct_digits_only, correct_digits_and_position = take_turn(code)
        show_results(correct_digits_and_position, correct_digits_only)
        turns += 1
        #check_correctness(turns, correct_digits_and_position)
